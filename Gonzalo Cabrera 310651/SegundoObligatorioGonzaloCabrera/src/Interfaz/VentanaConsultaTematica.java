    ...
    private void actualizarListaPostulantes(String unTema){
        ...
        for (int i = 0; i < sistema.getListaPostulantes().size(); i++) {
            for (int j = 0; j < sistema.getListaPostulantes().get(i).getExperiencias().size(); j++) {
                if (sistema.getListaPostulantes().get(i).getExperiencias().get(j).getTema().getNombre().equals(unTema) &&
                        sistema.getListaPostulantes().get(i).getExperiencias().get(j).getNivel() >= 5) {
                    String agregar = sistema.getListaPostulantes().get(i).getNombre() + "-" + sistema.getListaPostulantes().get(i).getCedula();
                    modelo.addElement(agregar);
                }
            }
        }
        lstPostulantesM5.setModel(modelo);
    }
    private void actualizarListaPuestos(String unTema){
        Sistema sistema = Sistema.getInstance();
        DefaultListModel<String> modelo = new DefaultListModel<>();
        for (int i = 0; i < sistema.getListaPuestos().size(); i++) {
            for (int j = 0; j < sistema.getListaPuestos().get(i).getTemas().size(); j++) {
                if (unTema.equals(sistema.getListaPuestos().get(i).getTemas().get(j).getNombre())) {
                    modelo.addElement(sistema.getListaPuestos().get(i).getNombre());
                }
            }
        }
        lstPuestosReq.setModel(modelo);
    }
